...
class MSCOCO(data.Dataset):
    ...
    def __init__(self, ann_path, data_path, vocab, data_transform=None):
        ...
        # Annotation IDs for MSCOCO dataset
        self.annotation_ids = list(COCO(ann_path).anns.keys())
    ...
    def __getitem__(self, image_idx):
        ...
        image_caption = self.annotation_obj.anns[annotation_idx]['caption']
        # Retrieving the image index corresponding to the annotation index
        image_idx = self.annotation_obj.anns[annotation_idx]['image_id']
        image_path = self.annotation_obj.loadImgs(
            image_idx)[0]['file_name']  # Recording the path of the image
        ...
        caption_tokens = nltk.tokenize.word_tokenize(
            str(image_caption).lower())
        image_target_caption = torch.Tensor([self.vocab(
            '<start>')] + [self.vocab(token) for token in caption_tokens] + [self.vocab('<end>')])
...
def get_data_loader(vocab, params, run_type):
    ...
    if run_type == 'train':
        dataset = MSCOCO(params['ann_path_train'],
                         params['data_path_train'], vocab, data_transform)
        ...
        dataset = MSCOCO(params['ann_path_test'],
                         params['data_path_test'], vocab, data_transform)
        data_loader = torch.utils.data.DataLoader(dataset=dataset, batch_size=params['batch_size'],
                                                  shuffle=False, num_workers=params['num_workers'],
                                                  collate_fn=create_batch)
    else:
        raise ValueError(
            'Please specify a valid run type for data loader. %s doesn\' exist.' % (run_type))
...
def create_caption_word_format(tokenized, vocab, flag_blue=False):
    ...
    for token in tokenized:
        curr_word = []
        # fix for np.int64 error
        # if isinstance(token, np.int64):
        #    idx = token
...
def test_model(cnn, rnn, optimizer, loss_function, data_loader, vocab, params, model_name_load, device, sub_batch_size=-1):  # noqa
    '''
    Function to test the model
    '''
    state_dict = torch.load(os.path.join(
        params['output_dir'], model_name_load + '.ckpt'), map_location=torch.device('cuda'))  # noqa
    ...
    for idx, (img_paths, image, caption, caption_len) in enumerate(data_loader, start=0):
        ...
        if device == 'cpu':
            ...
            raise ValueError(
                'Please specify a valid device from ["cpu", "gpu"].')
        ...
        target_words = create_caption_word_format(
            original_sentence_tokenized, vocab, True)
    ...
    for img_nm in target_caption_full.keys():
        ...
        for j in range(len(candidate_caption_full[img_nm])):
            b1 += corpus_bleu([target_caption_full[img_nm]], [candidate_caption_full[img_nm]
                                                              [j]], weights=(1.0, 0.0, 0.0, 0.0), smoothing_function=sf.method4)
            b2 += corpus_bleu([target_caption_full[img_nm]], [candidate_caption_full[img_nm]
                                                              [j]], weights=(0.5, 0.5, 0.0, 0.0), smoothing_function=sf.method4)
            b3 += corpus_bleu([target_caption_full[img_nm]], [candidate_caption_full[img_nm][j]],
                              weights=(0.34, 0.33, 0.33, 0.0), smoothing_function=sf.method4)
            b4 += corpus_bleu([target_caption_full[img_nm]], [candidate_caption_full[img_nm][j]],
                              weights=(0.25, 0.25, 0.25, 0.25), smoothing_function=sf.method4)

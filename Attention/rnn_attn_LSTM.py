...
class RNN_Attn(nn.Module):
	...
	def __init__(self, embed_dim, nos_filters, attention_dim, num_hidden_units, vocab_size, num_layers):
		...
		self.unit = nn.LSTM(2 * embed_dim, num_hidden_units, num_layers, batch_first=True)  # GRU(2*embed_dim, num_hidden_units, num_layers, batch_first=True) # 
		...
		self.init_c = nn.Linear(nos_filters, num_hidden_units)  # linear layer to find initial cell state of LSTMCell

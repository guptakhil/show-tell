...
parser.add_argument('--nos_cnn_filters', type=int, default=config['nos_cnn_filters'], help="number of channels in the final features of the CNN")
parser.add_argument('--attn_dim', type=int, default=config['attn_dim'], help="dimensions of the attention feature of the CNN")
...
parser.add_argument('--alpha_c', type=float, default=1.0, help="Weighting term for Doubly Stochastic Attention")
...
rnn = RNN(params['embedding_length'], params['nos_cnn_filters'], params['attn_dim'], params['num_hidden_units'], len(vocab), params['num_layers'])
...
if params['is_training']:
	...
	for epoch in range(params['num_epochs']):
		...
		for idx, (_, image, caption, caption_len) in enumerate(train_data_loader):
			...
			rnn_tokenized_sentence, alphas = rnn(cnn_feature, caption, caption_len)
			loss = loss_fn(rnn_tokenized_sentence, target_caption)
			loss += params['alpha_c'] * ((1. - alphas.sum(dim=1)) ** 2).mean() # Doubly Stochastic Attention
...
if params['is_testing']:
	...
	for idx, (img_paths, image, caption, caption_len) in enumerate(test_data_loader, start = 0):
		...
		target_caption = nn.utils.rnn.pack_padded_sequence(caption, caption_len, batch_first=True)[0]
		cnn_feature = cnn(image)
		rnn_tokenized_sentence, _ = rnn(cnn_feature, caption, caption_len)
		loss = loss_fn(rnn_tokenized_sentence, target_caption) # Teacher forced loss
		test_loss.append(loss.data.item())

		rnn_tokenized_sentence_prediction = rnn.sentence_index(cnn_feature, vocab)
